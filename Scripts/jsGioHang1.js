                ...
                success: function (result) {
                    ...
                    $.each(result, function (i, item) {
                        var R = "<tr>"
                            ...
                            + "<td>" + item.TenSP + "</td>"
                            + "<td><input onkeypress='return (event.charCode == 8 || event.charCode == 0) ? null : event.charCode >= 48 && event.charCode <= 57' type='number' min='1' id='" + item.MaSP + "' class='txtSLGH' value=" + item.SoLuong + " /><a class='fa fa-upload btn-Update' onclick='CapNhat(" + item.MaSP + ")' aria-hidden='true'></a><a  class='fa fa-trash-o btn-Delete' aria-hidden='true' onclick=' XoaSP(" + item.MaSP + ")'></a></td>"
                            + "<td>" + item.GiaSP + "</td>"
                            + "<td>" + item.TongTien + "</td>"
                            + "</tr>"
